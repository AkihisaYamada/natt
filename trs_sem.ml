...
let theory env t = 
  match t with
    | Builtin((loc,"AC"),funlist) ->
    List.fold_left check_binary env funlist
    | Builtin((loc,"C"),funlist) ->
    List.fold_left check_binary env funlist
    | Builtin((loc,"A"),funlist) ->
    List.fold_left check_binary env funlist
...
let rule env = function
  ...
  | DistRew(l,rs) ->
    let env = term env l in
    List.fold_left (fun env p -> term env (snd p)) env rs
...
let extra_var_conditions env = function
  ...
  | DistRew(l,d) ->
    let lvars = new_vars_of_term env StringSet.empty l in
    List.iter (fun p -> check_vars_of_term env lvars (snd p)) d

let post_check env d = 
  match d with
    | VarDecl l -> ()
    | TheoryDecl l -> ()
    | RulesDecl l -> (* AY: List.iter (extra_var_conditions env) l *)()
