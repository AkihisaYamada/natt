...
let rec smt_order e1 e2 =
	...
	| PB e1, PB e2	-> smt_if e1 (Cons(LB true, e2)) (Cons(smt_not e2, LB false))
	| Vec(e1::es1), Vec(e2::es2) ->
		smt_if (smt_for_all2 (>=^) es1 es2) (smt_order e1 e2) not_ordered
	| LI i1, PB e2 	->
		if i1 > 1 then strictly_ordered
		else if i1 = 1 then Cons(LB true, smt_not e2)
		else if i1 = 0 then Cons(smt_not e2, LB false)
		else not_ordered
	| PB e1, LI i2 	->
		if 0 > i2 then strictly_ordered
		else if 0 = i2 then Cons(LB true, e1)
		else if 1 = i2 then Cons(e1, LB false)
		else not_ordered
	| _ ->
		Delay(fun solver ->
